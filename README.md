...
- **Звук (`t`)**: проигрывает мелодию как фортепианные тоны (синтез, `rodio`)
  без эмуляции клавиш. Громкость по умолчанию 50%, регулируется `+`/`-`.
  Остановка — тем же глобальным стоп-хоткеем (`Esc`).
...
| `t`          | проиграть мелодию **звуком** (стоп — `Esc`)|
| `+` / `-`    | громкость звука                            |
...
аккорды `[...]` звучат одновременно. Абсолютная октава условна (мелодия
звучит верно относительно себя). Это **не** эмуляция клавиш — просто звук
для предпрослушивания. Стоп — глобальный стоп-хоткей.
...
- Лог: `Documents/Piano/piano-tui.log`.
